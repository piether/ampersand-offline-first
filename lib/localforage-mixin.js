		...
		if(model.isCollection) {
			...
			localforage.iterate(function(value,key,index) {
				...
				if(value.time===undefined && navigator.onLine)
					saveToServer.push({key:key,data:value});
...
var fetchOnlineData = function(method,model,options) {
	return new Promise(function(resolve,reject) {
      ...
      optionsCopy.success = function (resp) {
        if (success) success(resp);
      	model.trigger('updating', model, resp, options);
      	resolve(resp);
      };
...
var compareOnAndOfflineData = function(local,online,method,model,options) {
	var updateServer = [];
	...
	function updateModel(model,data,responseFromOnline) {
		...
		if(model && data && (!model.time || data.time>model.time)) {
			...
			if(responseFromOnline) {
				...
				if(model.offline || model.time) localforage.setItem(result(model, 'url'), model.toJSON());
	...
	if(updateServer.length) {
		...
		if(options.complete)
			syncOptions.success = function() {
				options.complete(model,model.toJSON(),options);
			};
		return sync.apply(this,['update',model,syncOptions]);
	// if options got a "complete"-callback, trigger it
	} else if(options.complete) options.complete(model,model.toJSON(),options);
};

// fetch
var read = function(method,model,options) {
	// make sure a collection does not reset, remove or merge content while fetching
	...
	return Promise.all([local,online]).then(function(data) {
		...
		return compareOnAndOfflineData(data[0],data[1],method,model,options);
	},function(err) {
...
var save = function(method,model,options) {
	...
	if (model.isState) {
		...
		if(offline) model.time = Date.now();
		else model.unset('time');

...
var read = function(method,model,options) {
	...
	options.success = function(res) {
		...
		function addOrMergeToCollection(doc) {
			var existing = model.filter(function(item) { return item[model.mainIndex] === doc[model.mainIndex]; })[0];
			if(existing) {
				existing.clear().set(doc);
			} else model.add(doc);
		}
...
var saveOnline = debounce(function(method,model,options) {
...
var saveOffline = debounce(function(model) {
...
var save = function(method,model,options) {
	if(!(model.doNotSave || (model.collection && model.collection.doNotSave))) {
		...
		if(doSync) saveOnline(method,model,options);
		else if (options.success) return options.success();
	}
};

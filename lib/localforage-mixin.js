	...
	if(!(model.doNotSave || (model.collection && model.collection.doNotSave))) {
		...
		if (model.isState) {
			...
			forEach(model._children, function (value, key) {
				if(Object.keys(model[key].changedAttributes()).length) changedAttributes.push(key);
      }, model);
		...
		if(doSync && navigator.onLine) {
			// image your server is bussy... and responds rather late
			// make sure that a file, which is BEING saved, will not get removed during the merge-function after a fetch-requests
			if(model.isState) {
				if(justBeenSaved.indexOf(model.getId())===-1) justBeenSaved.push(model.getId());
      } else

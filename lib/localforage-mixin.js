...
// REMOVE UNDEFINED VALUES FROM AN EXISTING MODEL
var removeUndefinedValues = function(model,update) {
	// unset properties which have an undefined value in their update
	var json = model.toJSON(),
		keysToRemove = [];
	for(var key in json) {
		if(json.hasOwnProperty(key)) {
			// unset the property if the updated prop is undefined 
			if(update[key]===undefined && typeof model._definition[key]==='object')
				keysToRemove.push(key);
			// deep update if this is a child property
			else if(typeof update[key]==='object' && !(update[key] instanceof Array))
				removeUndefinedValues(model[key],update[key]);
		}
	}
	model.unset(keysToRemove);
};

// UPDATE ALL PROPERTIES OFF AN EXISTING MODEL
var updateModelProperties = function(model,update) {
	model.set(update);
	removeUndefinedValues(model,update);
};
...
var read = function(method,model,options) {
	...
	options.success = function(res) {
		...
		function addOrMergeToCollection(doc) {
			...
			if(existing) updateModelProperties(existing,doc);
			else model.add(doc);
		}
		...
		// if this is a model, merge properties
			...
			updateModelProperties(model,res);
	};
...
// check if a changed property is also a property of the toJSON-object and is not just a session property
var allowModelToSave = function(model) {

	function getJSONKeys(model,prefix) {
		var attr = [];//Object.keys(model).map(function(item) { return (prefix?prefix+'.':'')+item; });
		// get changed properties of the children
		forEach(model, function (value, key) {
			if(typeof value==='object' && !(value instanceof Array))
				attr = attr.concat(getJSONKeys(model[key],key));
			else
				attr.push((prefix?prefix+'.':'')+key);
    }, model);
		return attr;
	}

	function getChangedProperties(model,prefix) {
		var changedAttributes = Object.keys(model.changedAttributes()).
			map(function(item) { return (prefix?prefix+'.':'')+item; });
		// get changed properties of the children
		forEach(model._children, function (value, key) {
			changedAttributes = changedAttributes.concat(getChangedProperties(model[key],key));
    }, model);
		return changedAttributes;
	}

	if (model.isState) {
		// get changedAttributes to compare them with the toJSON-to-be-saved
		var changedAttributes = getChangedProperties(model),//Object.keys(model.changedAttributes()),
			json = getJSONKeys(model.toJSON()),
			doSync = false;
		
		// only sync to server when at least one of the changed attributes does appear in the toJSON-output
		for(var i=0; i<changedAttributes.length; i++)
			if(!(changedAttributes[i] in json) && (changedAttributes[i]!=='time'))
				doSync = true;
		return doSync;
	}
	return true;
